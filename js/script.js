...
const timeAgo = (time) => {
    ...
    const lastPost = new Date(time); // date of the last activity on a topic
...
const fetchData = async () => {
    try {
        ...
        const data = await res.json();// returns a promise meaning we have to wait for it
...
const showLatestPosts = (data) => {
    ...
    postsContainer.innerHTML = topics.map((item) => {
        ...
        <tr>
            ...
            <td>
                ${posts_count - 1}  <!-- This will display the number of replies to the topic -->
            </td>
